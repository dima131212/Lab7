...
public class DatabaseMovieManager {
    ...
    public void deleteMovie(Long movieId) throws SQLException {
        ...
        try (
            // Получаем operator_id и coordinates_id
            ...
            // Получаем location_id по operator_id
            ...
            // Удаляем location (вручную, если нужна явная очистка)
            ...
            // 1. Получаем operator_id и coordinates_id
            ...
            // 2. Получаем location_id, если есть operatorId
            if (operatorId != null) {
            ...
            // 3. Удаляем сам фильм (все связанные объекты с ON DELETE CASCADE удалятся, кроме location)
            deleteMovieStmt.setLong(1, movieId);
            deleteMovieStmt.executeUpdate();

            // 4. Удаляем location вручную (если хотим полностью очистить)
            if (locationId != null) {
